...
const colorDisabledClasses: Record<string, string> = {
  red:   "bg-gray-300 border-gray-400 cursor-not-allowed",
  blue:  "bg-gray-300 border-gray-400 cursor-not-allowed",
};

export default function Button({
    className='', color, disabled=false, children, onClick
}: ButtonProps) {
    const colorClassName = disabled === false ? colorClasses[color] || colorClasses['red'] : colorDisabledClasses[color] || colorDisabledClasses['red']
    return <button 
                disabled={disabled}
                onClick={onClick}
